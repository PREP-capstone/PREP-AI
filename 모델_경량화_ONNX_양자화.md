...
## 실측 결과 (2026-08-23, 구버전 `best_healthcare_model_2line` 체크포인트 기준)
...
## 재실측 (2026-09-03, 구글 플레이 397건 병합 후 재학습된 체크포인트 기준, Avg Macro F1 0.7129 — [[학습_기록]] 참고)

인코더/헤드 구조는 동일하고 가중치만 바뀐 재학습 체크포인트로 파이프라인을
다시 돌렸다 (Windows 로컬, CPU).

### 파일 크기

| 형태 | 크기 | 비고 |
|---|---|---|
| ONNX fp32 | 420.0MB | 8/23 체크포인트와 사실상 동일 (구조 불변) |
| ONNX int8 | 105.7MB | 동적 양자화, **-74.8%** |

파일 크기는 구버전과 사실상 동일 — 구조가 안 바뀌었으니 당연한 결과. 이번엔
Windows 환경이라 macOS 기준 피크 RSS 재측정은 하지 않았음(구조 동일하므로
위 8/23 수치가 여전히 유효한 추정치로 봐도 무방).

### 정확도(수치 동등성) 검증

실제 앱 설명문 3건(수면/정신건강 매칭/유전자검사 관련 문장)으로 PyTorch ↔
ONNX(fp32) ↔ ONNX(int8) 로짓을 직접 비교:

- **fp32**: 3건 모두 PyTorch와 `max|diff| = 0.0000`으로 완전히 일치 — export
  파이프라인 자체는 이번 체크포인트에서도 정상.
- **int8**: 로짓 차이 `max|diff|` 0.18~0.46(category) 수준 — 8/23 결과와
  비슷한 정상 범위의 양자화 오차. 3건 모두 argmax 라벨은 fp32와 동일했다.
- 참고: 손으로 즉석에서 지어낸 문장 일부(예: 온라인 병원 예약 플랫폼 예외
  케이스)는 확신도가 낮게 나왔는데, 이는 ONNX 변환 문제가 아니라 원본 PyTorch
  모델 자체의 예측이 그렇게 나온 것(직접 대조로 확인) — Avg F1 0.71 수준
  모델의 정상적인 한계.
...
- ~~다른 데스크탑에서 학습 중인 개선 버전(category_2 정확도 향상)이 완성되면,
  ...
  생성한다.~~ → 완료 (2026-09-03, 위 "재실측" 섹션 참고)
